...
class CoordinateFunctionSpecifierParserTest extends FunSpec with PropertyChecks {
	import CoordinateFunctionSpecifierParser.parser
	...
	describe ("The function produced by AdjacentSpacesSpecifierParser.parser") {
		describe ("when the specifier is \"true\"") {
			val parserResult = parser.parse("true").get.value
		...
		describe ("when the specifier is \"false\"") {
			val parserResult = parser.parse("false").get.value
		...
		describe ("when the specifier is \"x==2\"") {
			val parserResult = parser.parse("x==2").get.value
		...
		describe ("when the specifier is \"y==2\"") {
			val parserResult = parser.parse("y==2").get.value
		...
		describe ("when the specifier is \"w==2\"") {
			val parserResult = parser.parse("w==2").get.value
		...
		describe ("when the specifier is \"h==2\"") {
			val parserResult = parser.parse("h==2").get.value
		...
		describe ("when the specifier is \"x + y == 2\"") {
			val parserResult = parser.parse("x + y == 2").get.value
		...
		describe ("when the specifier is \"x * 2 == y\"") {
			val parserResult = parser.parse("x * 2 == y").get.value
			it ("returns true when x * 2 == y") {
				forAll{(x:Int, w:Int, h:Int) => {
					val y = x * 2
					assert(parserResult(x, y, w, h) )
		...
		describe ("when the specifier is \"x / 2 == 0\"") {
			val parserResult = parser.parse("x / 2 == 0").get.value
		...
		describe ("when the specifier is \"(x + y) % 2 == 0\"") {
			val parserResult = parser.parse("(x + y) % 2 == 0").get.value
			it ("returns true when x is even") {
		...
		describe ("when the specifier is \"x == 0 || y == 0\"") {
			val parserResult = parser.parse("x == 0 || y == 0").get.value
		...
		describe ("parentetical around a boolean part") {
			val parserResult = parser.parse("(x < 0 && y < 0) || w == h").get.value
	...
	describe ("AdjacentSpacesSpecifierParser.parser") {
		it ("does not accept an integer-returning expression") {
			parser.parse("x + y").fold[Unit]({(parser, idx, extra) =>
					// success
				}, {(res, idx) =>
		...
		it ("does not accept a variable 'b'") {
			parser.parse("b == 0").fold[Unit]({(parser, idx, extra) =>
					// success
				}, {(res, idx) =>
					fail("Parse succeeded: " + res)
			})
		...
		it ("does not accept a variable 'xy'") {
			parser.parse("xy == 0").fold[Unit]({(parser, idx, extra) =>
					// success
				}, {(res, idx) =>
					fail("Parse succeeded: " + res)
			})
